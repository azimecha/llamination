...
namespace Azimecha.Llamination.TextGeneration {
    public class TokenBasedPromptInterface<TModel, TState> : IPromptInterface
        where TModel : class, ITokenBasedLLM, ISelectableStateModel<TState> where TState : IDisposable 
    {
        private TModel _mdl;
        ...
        private TState _stateOriginal;

        public TokenBasedPromptInterface(TModel mdl) {
            _mdl = mdl;

            _lstSamplers.AddRange(_mdl.DefaultSamplers);
